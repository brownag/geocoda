...
#' Discretize 3D Block into Sub-Points
#'
#' Subdivide a 3D block into internal sub-points for covariance calculation in 3D space.
#' Uses regular grid points centered within the block for uniform coverage.
#'
#' @param block_centers Data frame with x, y, z columns (block center coordinates)
#' @param block_size Vector c(dx, dy, dz) specifying block dimensions (same units as coordinates)
#' @param discretization Vector c(nx, ny, nz) specifying number of sub-points per dimension
#'   (default c(3, 3, 3) gives 27 sub-points per block)
#'
#' @return Data frame with columns:
#'   - x, y, z: Sub-point coordinates
#'   - block_id: Index linking sub-point to parent block
#'
#' @details
#' **Discretization strategy (3D extension):**
#' - Blocks are subdivided into regular 3D grid of sub-points
#' - Sub-points are centered within each sub-cell (not at corners)
#' - Example: 3×3×3 discretization creates 27 evenly-spaced sub-points per block
#' - Total sub-points = nrow(block_centers) × (nx × ny × nz)
#' - Smaller discretization (3×3×3) recommended for 3D to manage computational cost
#'
#' **Block-support covariance computation:**
#' Block covariances are computed via triple integration using sub-points:
#' ```
#' C_BBB(h) = (1/V³) ∑∑∑ C(u_i - v_j - w_k) (approximate via sub-points)
#' ```
#'
#' @examples
#' \dontrun{
...
#'
#' @export
gc_discretize_block_3d <- function(block_centers, block_size, discretization = c(3, 3, 3)) {
  stopifnot(is.data.frame(block_centers), all(c("x", "y", "z") %in% names(block_centers)))
  stopifnot(is.numeric(block_size), length(block_size) == 3, all(block_size > 0))
  stopifnot(is.numeric(discretization), length(discretization) == 3, all(discretization >= 2))
  ...
  x_offsets <- seq(-dx_block / 2 + dx_sub / 2, dx_block / 2 - dx_sub / 2, length.out = nx)
  y_offsets <- seq(-dy_block / 2 + dy_sub / 2, dy_block / 2 - dy_sub / 2, length.out = ny)
  z_offsets <- seq(-dz_block / 2 + dz_sub / 2, dz_block / 2 - dz_sub / 2, length.out = nz)
  ...
  sub_points_list <- lapply(seq_len(nrow(block_centers)), function(i) {
    ...
    data.frame(
      x = block_centers$x[i] + grid$x_offset,
      y = block_centers$y[i] + grid$y_offset,
      z = block_centers$z[i] + grid$z_offset,
      block_id = i
    )
  })
...
#' Average Sub-Point ILR Values to 3D Block Support
#'
...
#' @param sim_result Simulation result data frame with sub-point locations and ILR values.
#'   Columns should include ILR simulation results (e.g., ilr1.sim1, ilr2.sim1, etc.)
#' @param sub_points Data frame from [gc_discretize_block_3d()] with block_id column
#'   linking each row to its parent 3D block
#'
#' @return Data frame with averaged ILR values per block per realization
#'   - Rows: One per 3D block
#'   - Columns: ilr1.sim1, ilr1.sim2, ..., ilr2.sim1, ilr2.sim2, ...
#'   (or equivalent MAF columns if MAF transformation was used)
#'
#' @details
#' **Averaging operation (3D extension):**
#' For each 3D block B and simulation S:
#' ```
#' ILR_B^S = mean(ILR_ijk^S : (i,j,k) ∈ sub-points of B)
#' ```
#'
#' This averaging creates the volume-variance relationship in 3D:
#' - Block variance = Point variance / (number of sub-points)
#' - Blocks are smoother than points (lower variance)
#' - Effect is more pronounced in 3D due to larger sub-point counts
#'
#' @examples
#' \dontrun{
...
#'
#' @export
gc_average_block_ilr_3d <- function(sim_result, sub_points) {
  stopifnot(is.data.frame(sim_result), is.data.frame(sub_points))
  stopifnot("block_id" %in% names(sub_points))
  stopifnot(nrow(sim_result) == nrow(sub_points))
